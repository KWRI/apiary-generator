...
abstract class AbstractParser
{
     ...
     */
    protected function getRouteResponse($route, $bindings, $headers = [], $parsedRoute = [])
    {
        $json = [];
        ...
        foreach (array_keys($bindings) as $key) {
            if (array_key_exists($key, $parsedRoute['parameters'])) {
                $bindings[$key] = $parsedRoute['parameters'][$key]['value'];
            }
        }

        if (!empty($rules) && !empty($bindings)) {
            $uri = GenerateDocumentationHelpers::getUri($rules, $bindings, $uri);

            if (!empty($parsedRoute)) {
                $json = GenerateDocumentationHelpers::buildJson($parsedRoute['parameters']);
            }
        }
        ...
        return $this->callRoute(array_shift($methods), $uri, $json, [], [], $headers, null);
    }
...
//        }
            ...
            if (!is_null($parameterType) && class_exists($parameterType->name)) {
                ...
                if (is_subclass_of($className, FormRequest::class)) {
                    ...
                    if (method_exists($parameterReflection, 'validator')) {
                        return $this->parseArrayRules($parameterReflection->validator()->getRules());
                    } else {
                        return $this->parseArrayRules($parameterReflection->rules());
                    }
                }
            }
        }
     ...
     * @return bool
     */
    protected function parseRule($rule, $ruleName, &$attributeData, $seed, $uri = null)
    {
        $resourse = null;
        if ($uri) {
            $resourse = explode('/', $uri);
            $resourse = end($resourse);
        }
        ...
        switch ($rule) {
            ...
            case 'after':
                ...
                $attributeData['description'][] =
                    Description::parse($rule)->with(date(DATE_RFC850, strtotime($parameters[0])))->getDescription();
            ...
            case 'digits':
                $attributeData['type'] = 'numeric';
                $attributeData['description'][] = Description::parse($rule)->with($parameters)->getDescription();
                $attributeData['value'] = 1; //$faker->randomNumber($parameters[0], true);
                break;
            case 'digits_between':
                $attributeData['type'] = 'numeric';
                $attributeData['description'][] = Description::parse($rule)->with($parameters)->getDescription();
            ...
            case 'required_with_all':
                $attributeData['description'][] =
                    Description::parse($rule)->with($this->fancyImplode($parameters, ', ', ' and '))->getDescription();
                break;
            ...
            case 'exists':
                ...
                $attributeData['description'][] =
                    Description::parse($rule)->with([Str::singular($parameters[0]), $fieldName])->getDescription();
                break;
            case 'active_url':
            ...
            case 'string':
                ...
                break;
            case 'integer':
                $attributeData['value'] = 1; //$faker->randomNumber();
                $attributeData['type'] = $rule;
                break;
            case 'numeric':
                $attributeData['value'] = 1; //$faker->randomNumber();
     ...
     * @return \Illuminate\Http\Response
     */
    abstract public function callRoute($method, $uri, $parameters = [],
    ...
    protected function parseArrayRules($rules)
    {
        $parsedRules = [];

        foreach ($rules as $key => $rule) {
            if (strpos($key, '*') !== false) {
                $parsedRules[str_replace('*', '0', $key)] = $rule;
            } else {
